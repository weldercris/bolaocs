  ...
  return (
    <div className="max-w-6xl mx-auto px-4 pt-32 pb-32 space-y-8">
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] bg-white h-[28rem] md:h-[32rem] flex items-center shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-gray-100 mb-12">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[40rem] h-[40rem] bg-ocean/5 rounded-full blur-[80px] pointer-events-none" />
        
        <img src="/brasil.jpg" alt="Brasil Background" className="absolute inset-0 w-full h-full object-cover opacity-5 saturate-0 mix-blend-multiply pointer-events-none" />

        <div className="relative z-10 flex flex-col w-full md:w-3/5 px-8 md:px-16">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-10 h-0.5 bg-ruby"></div>
            <h2 className="text-sm text-ruby font-bold uppercase tracking-widest">{greeting},</h2>
          </div>
          ...
          <div className="flex flex-wrap gap-4">
            <Link to="/palpites" className="btn-gold px-8 py-4 text-lg shadow-gold/20 shadow-lg">
              <Target size={20} /> Fazer Palpites
            </Link>
            <Link to="/ranking" className="btn-ghost px-8 py-4 text-lg bg-white/50 backdrop-blur-md">
              <Trophy size={20} /> Ver Ranking
            </Link>
          </div>
        </div>

        <div className="absolute right-0 bottom-0 h-full w-2/5 hidden md:flex items-end justify-end pointer-events-none pr-8">
          <img 
            src="/brazil.png" 
            alt="Brasil Fundo" 
            className="h-[100%] object-contain object-bottom translate-y-4 translate-x-4 opacity-90 drop-shadow-2xl"
          />
        </div>
      </div>
              ...
              return (
                <div key={player.id} className="relative z-10 flex items-center gap-4 animate-in">
                  ...
                  <div className="card-hover flex-1 p-5 flex items-center gap-4">
                    <span className="text-3xl bg-background-gray rounded-full p-2 border border-gray-100">{player.avatar_emoji}</span>
